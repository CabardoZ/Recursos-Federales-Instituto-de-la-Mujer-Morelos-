# -*- coding: utf-8 -*-
"""PROYECTO III. IMM MORELOS FINAL

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1adcf6eQuFjpdwf_fcth29_wxFFr4scoZ
"""
...
df_descripcion = pd.read_excel('DATA IMM MORELOS.xlsx', sheet_name='DESCRIPCIÓN PROGRAMAS FEDERALES')

df_monitoreo = pd.read_excel('DATA IMM MORELOS.xlsx', sheet_name='MONITOREO DEL PRESUPUESTO')

print(df_descripcion.head())
...
programa_seleccionado = st.selectbox('Selecciona un programa:', df_descripcion['PROGRAMA'].unique())

programa_info = df_descripcion[df_descripcion['PROGRAMA'] == programa_seleccionado]

st.subheader("Objetivos específicos")
st.write(programa_info['OBJETIVOS ESPECÍFICOS'])
...
fig = px.pie (df_recursos,
              names='PROGRAMA',
              values='RECURSOS',
              title="DISTRIBUCIÓN DE RECURSOS POR PROGRAMA FEDERAL",
              hover_data=['PROGRAMA'])
...
df_presupuesto = pd.read_excel('DATA IMM MORELOS.xlsx', sheet_name='MONITOREO DEL PRESUPUESTO')
...
# Cargar los datos desde la hoja "RECURSOS TOTAL"
df_recursos = pd.read_excel('DATA IMM MORELOS.xlsx', sheet_name='RECURSOS TOTAL')
...
fig = px.pie(df_recursos,
             names='PROGRAMA',
             values='RECURSOS',
             title="Distribución de los Recursos Totales por Programa",
             hover_data=['PROGRAMA'])

# Asignar los colores personalizados para los programas
fig.update_traces(
    marker=dict(
        colors=[
            '#c72c18',  # PAIMEF en Naranja
            '#643ee6',  # PROABIM en Morado
            '#489f3b'    # FOBAM en Verde
        ]
    )
)
...
app.layout = html.Div([
    html.H1("Distribución de Recursos por Programa"),
    dcc.Graph(figure=fig)  # Incluir el gráfico de pastel
])
...
# Gráfico de barras de monitoreo
fig_bar = px.bar(df_monitoreo,
                 x=['EJECUTADO', 'POR EJECUTAR'],
                 y='PROGRAMA',
                 orientation='h',
                 barmode='stack',
...
fig_bar.update_traces(
    hovertemplate='%{y}<br>%{x}<br>%{customdata[0]}<br>%{customdata[1]}<br>%{customdata[2]}',
...
fig_violen_bar = px.bar(df_long,
                        ...
                        color='NUMERO_DE_PERSONAS',
                        color_continuous_scale='YlOrRd',
                        orientation='v',
                        title="Tipos de Violencia por Grupo",
...
fig_violen_bar.update_traces(
    hovertemplate='<b>%{x}</b><br>Grupo: %{customdata[1]}<br>Número de Personas: %{y}',
    customdata=df_long[['GRUPO']].values
)
...
app.layout = html.Div([
    html.H1("Monitoreo de Programas Federales y Violencia"),

    # Gráfico de pastel
    html.H3("Distribución de Recursos por Programa"),
    dcc.Graph(id='grafico-pie', figure=fig_pie),

    # Gráfico de barras de recursos
    html.H3("Avance por Programa Federal"),
    dcc.Graph(id='grafico-barras', figure=fig_bar),

    # Gráfico de barras de violencia por grupo
    html.H3("Tipos de Violencia por Grupo"),
...
# Formatear el hover para que muestre las cantidades seguidas del grupo, como "5875 mujeres"
fig_violen_bar.update_traces(
    hovertemplate='<b>%{x}</b><br>%{y} %{customdata[0]}',  # Aquí se muestra la cantidad seguida del grupo
...
fig_bar = px.bar(df_monitoreo,
                 ...
                 color_discrete_map={  # Colores personalizados
                     'PAIMEF': 'orange',     # Naranja para PAIMEF
                     'PROABIM': 'purple',    # Morado para PROABIM
                     'FOBAM': 'green'        # Verde para FOBAM
                 })

# Ajuste en el hover: queremos que en el hover se muestre el nombre del programa y la descripción correspondiente
fig_bar.update_traces(
    hovertemplate='%{y}<br>%{x}',  # Solo mostrar el monto correspondiente
    customdata=df_monitoreo[['DESCRIPCIÓN RECURSOS HUMANOS', 'DESCRIPCIÓN RECURSOS MATERIALES', 'DESCRIPCIÓN METAS DEL PROYECTO']].values  # Usar la columna de descripción
)
...
# Estructura de la aplicación
app.layout = html.Div([
    html.H1("MONITOREO DE RECURSOS FEDERALES DEL INSTITUTO DE LA MUJER PARA EL ESTADO DE MORELOS"),

    # Gráfico de pastel
    html.H3("DISTRIBUCIÓN DE RECURSOS POR PROGRAMA"),
    dcc.Graph(id='grafico-pie', figure=fig_pie),  # Inicialmente vacío

    # Menú desplegable para seleccionar el rubro de recursos para el gráfico de barras
    html.Label('Selecciona el rubro de recursos para el gráfico de barras:'),
    dcc.Dropdown(
        id='recurso-dropdown',
        options=[
    ...
    # Gráfico de barras de recursos
    html.H3("Avance por Programa Federal"),
    dcc.Graph(id='grafico-barras', figure=fig_bar),  # Inicialmente vacío
...
def actualizar_grafico_barras(rubro_seleccionado):
    ...
    fig_bar.update_traces(
        hovertemplate='%{y}<br>%{x}',  # Mostrar solo el monto seleccionado
        customdata=df_monitoreo[['DESCRIPCIÓN RECURSOS HUMANOS', 'DESCRIPCIÓN RECURSOS MATERIALES', 'DESCRIPCIÓN METAS DEL PROYECTO']].values
    )
...
# Cargar los datos desde la hoja 'MUJERES'
...
# Definir los colores para cada tipo de centro
color_map = {
    'Centro de Atención Externa': '#f9200d',  # Rojo para Centro de Atención Externa
    'Línea Segura': '#f9200d',  # Rojo para Línea Segura
    'Unidad Móvil': '#f9200d',  # Rojo para Unidad Móvil
    'Centro para el Desarrollo de las Mujeres': '#3d1fde'  # Azul para Centro para el Desarrollo de las Mujeres
}
...
# Estructura de la aplicación
app.layout = html.Div([
    html.H1("Mujeres Atendidas y Alcanzadas por Centro de Atención"),

    # Gráfico de barras
    dcc.Graph(id='grafico-barras', figure=fig)
])
...
# Convertir las columnas numéricas a tipo numérico y manejar errores en 'MONITOREO DEL PRESUPUESTO'
df_monitoreo['EJECUTADO'] = pd.to_numeric(df_monitoreo['EJECUTADO'], errors='coerce').fillna(0)
df_monitoreo['POR EJECUTAR'] = pd.to_numeric(df_monitoreo['POR EJECUTAR'], errors='coerce').fillna(0)
df_monitoreo['TOTAL'] = pd.to_numeric(df_monitoreo['TOTAL'], errors='coerce').fillna(0)
...
# Limpiar los nombres de las columnas en 'MUJERES' y convertir las mujeres atendidas a tipo numérico
df_mujeres.columns = ['Centro', 'Mujeres']
df_mujeres['Mujeres'] = df_mujeres['Mujeres'].str.extract('(\d+)').astype(float)

# Filtrar las filas relevantes en 'MUJERES'
centros_atencion_externa = df_mujeres[df_mujeres['Centro'].str.contains('Centro de Atención Externa')]
linea_segura = df_mujeres[df_mujeres['Centro'] == 'Línea Segura']
unidad_movil = df_mujeres[df_mujeres['Centro'] == 'Unidad Móvil']
centros_para_desarrollo_mujeres = df_mujeres[df_mujeres['Centro'].str.contains('Centro para el Desarrollo de las Mujeres')]

# Concatenar los DataFrames de 'MUJERES'
df_final = pd.concat([centros_atencion_externa, linea_segura, unidad_movil, centros_para_desarrollo_mujeres])
...
# Crear el gráfico de barras para mujeres atendidas
...
# Crear el gráfico de barras para monitoreo de recursos
fig_bar = px.bar(df_monitoreo,
                 x=['EJECUTADO', 'POR EJECUTAR'],
                 y='PROGRAMA',
                 orientation='h',
...
# Crear el gráfico de pastel para la distribución de recursos por programa
fig_pie = px.pie(df_recursos,
                 names='PROGRAMA',
                 values='RECURSOS',
                 title="Distribución de Recursos por Programa",
                 labels={'TOTAL': 'Monto', 'PROGRAMA': 'Programa'},
                 color='PROGRAMA',
                 color_discrete_map={  # Colores personalizados
                     'PAIMEF': 'orange',
                     'PROABIM': 'purple',
...
# Convertir los datos de PREVENCIÓN a formato largo para crear el gráfico de barras de prevención
df_long = df_prevention.melt(id_vars=['VIOLENCIA'],
                             value_vars=['NIÑAS ALCANZADOS', 'NIÑOS ALCANZADOS', 'MUJERES', 'HOMBRES'],
                             var_name='GRUPO',
                             value_name='NUMERO_DE_PERSONAS')
...
# Definir los colores para los grupos de prevención
color_map_prevention = {
    'NIÑAS': '#ea3f24',
    'NIÑOS': '#ef6141',
    'MUJERES': '#f87f5c',
    'HOMBRES': '#ffc598'
...
fig_violen_bar = px.bar(df_long,
                        ...
                        y='NUMERO_DE_PERSONAS',
                        color='GRUPO',
                        color_discrete_map=color_map_prevention,
                        orientation='v',
                        title="PERSONAS ALCANZADAS POR PROCESOS DE PREVENCIÓN",
                        labels={'NUMERO_DE_PERSONAS': 'Número de Personas', 'VIOLENCIA': 'Tipo de Violencia'},
                        hover_data={'NUMERO_DE_PERSONAS': True, 'GRUPO': True})

# Formatear el hover para que muestre las cantidades seguidas del grupo
fig_violen_bar.update_traces(
    hovertemplate='<b>%{x}</b><br>%{y} %{customdata[0]}',  # Aquí se muestra la cantidad seguida del grupo
    customdata=df_long[['GRUPO']].values  # Personalizamos el hover para mostrar el grupo asociado
)
...
# Estructura de la aplicación
app.layout = html.Div([
    html.H1("Monitoreo de Recursos y Mujeres Atendidas en Morelos"),

    # Gráfico de pastel para la distribución de recursos
    html.H3("Distribución de Recursos por Programa"),
    dcc.Graph(id='grafico-pie', figure=fig_pie),

    # Gráfico de barras para mujeres atendidas
    html.H3("Mujeres Atendidas y Alcanzadas por Centro de Atención"),
    dcc.Graph(id='grafico-mujeres', figure=fig_mujeres),

    # Gráfico de barras para monitoreo de presupuesto de recursos
    html.H3("Avance por Programa Federal"),
    dcc.Graph(id='grafico-barras', figure=fig_bar),

    # Gráfico de barras de prevención por grupo
    html.H3("Tipos de Violencia por Grupo"),
    dcc.Graph(id='grafico-violencia', figure=fig_violen_bar)
])
...
# Cargar los datos desde la hoja 'NÚMERO DE SERVICIOS'
...
fig_pie = px.pie(df_recursos,
                 ...
                 color='PROGRAMA',
                 color_discrete_map={  # Colores personalizados
                     'PAIMEF (Programa de Apoyo a las Instancias de las Mujeres en las Entidades Federativas)': '#c72c18',
                     'PROABIM (Programa para el Adelanto, Bienestar e Igualdad de las Mujeres)': '#643ee6',
                     'FOBAM (Fondo para el Bienestar y el Avance de las Mujeres)': '#489f3b'
                 })
...
for servicio in servicios_columns:
    fig_servicios.add_trace(go.Bar(
        ...
        name=servicio,
        marker=dict(color=px.colors.qualitative.Set1[servicios_columns.index(servicio) % len(px.colors.qualitative.Set1)])
    ))
...
app.layout = html.Div([
    ...
    # Gráfico de barras de recursos
    html.H3("RECURSOS FEDERALES POR PARTIDA PRESUPUESTAL"),
    dcc.Graph(id='grafico-barras', figure=fig_bar),

    # Gráfico de barras de mujeres atendidas
    html.H3("MUJERES ALCANZADAS POR LOS CDM Y CAE DEL INSTITUTO DE LA MUJER PARA EL ESTADO DE MORELOS"),
    dcc.Graph(id='grafico-mujeres', figure=fig_mujeres),

    # Gráfico de barras de violencia por grupo
    html.H3("PERSONAS ALCANZADAS POR PROCESOS DE PREVENCIÓN"),
    dcc.Graph(id='grafico-violencia', figure=fig_violen_bar),

    # Gráfico de barras apiladas de servicios brindados
    html.H3("NÚMERO DE SERVICIOS BRINDADOS 2023 POR CENTRO DE ATENCIÓN EXTERNA"),
...
# Cargar los datos de georeferenciación
...
fig_map.update_layout(
    ...
    mapbox_zoom=7,  # Ajustar el zoom para que se vea todo Morelos
...
# Estructura de la aplicación
app.layout = html.Div([
    html.H1("MAPA DE GEOREFERENCIACIÓN DE SERVICIOS"),

    # Mapa
    dcc.Graph(id='mapa-georeferencia', figure=fig_map)
])
...
# Cargar los datos de georeferenciación (ya hemos definido este mapa antes)
df_georef = pd.read_excel('DATA IMM MORELOS.xlsx', sheet_name='SERVICIOS DE GEORREFERENCIA I')
...
app.layout = html.Div([
    ...
    html.H3("GEORREFERENCIACIÓN DE SERVICIOS EN MORELOS"),
...
# Crear el gráfico de barras apiladas 3D para el número de servicios con la nueva paleta de colores
fig_servicios = go.Figure()
...
# Paleta de colores personalizados para los servicios
colores_servicios = {
    'TANATOLOGÍA': '#d92714',
    'OTRO': '#fdb184',
...
# Eliminar la primera columna innecesaria (la columna combinada 'TIPO' y 'MODALIDAD')
df_violencia = df_violencia.drop(columns=['Unnamed: 0'])
...
# Eliminar la leyenda de la fuente (si se encuentra dentro de los 'annotations')
fig_violencia_barras.update_layout(
    annotations=[]  # Eliminar cualquier texto adicional como la fuente
)

# Ajustar la visualización del gráfico
fig_violencia_barras.update_layout(
    xaxis_title="Número de Casos de Violencia",
    yaxis_title="Modalidad de Violencia",
    legend_title="Tipo de Violencia",
    title="MODALIDAD Y TIPOS DE VIOLENCIA EN MUNICIPIOS",
    showlegend=True,
    height=600,
    xaxis={'tickformat': ',d'},  # Formato de números enteros en el eje X
    template='plotly_white'
)

# Mostrar el gráfico
fig_violencia_barras.show()
...
# El resto del código para el gráfico de barras de violencia
df_violencia.columns = ['TIPO', 'PSICOLOGÍCA', 'FÍSICA', 'ECONÓMICA', 'SEXUAL', 'PATRIMONIAL', 'FAMILIAR',
                        'LABORAL Y DOCENTE', 'COMUNITARIA', 'INSTITUCIONAL', 'POLÍTICA', 'DIGITAL Y MEDIÁTICA', 'FEMINICIDA']
...
color_map_tipo = {
    'LÍNEA SEGURA': '#D32F2F',
    'ÚNIDAD MÓVIL': '#F57C00',
    'CAE JONACATEPEC': '#FF9800',
    'CAE HUEYAPAN ': '#FF5722',
    'CAE XOCHITEPEC': '#E64A19',
    'CAE EMILIANO ZAPATA': '#D84315',
    'CAE TEMIXCO': '#F4511E',
    'CAE TETECALA': '#FF7043',
    'CJM CUERNAVACA': '#E53935',
    'CJM YAUTEPEC': '#C62828',
    'CAE CUERNAVACA ': '#D32F2F',
    'CAE YECAPIXTLA': '#B71C1C',
    'CAE AYALA':'#9c1d10',
    'CAE JIUTEPEC': '#f8230f'
}
...
app.layout = html.Div([
    ...
    html.H3("MODALIDAD Y TIPOS DE VIOLENCIA EN MUNICIPIOS"),
